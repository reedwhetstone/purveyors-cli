...
the SvelteKit server. The naive approach is to keep those functions inline in
`tools.ts` alongside the Vercel AI SDK Zod schemas — but that means every new
feature requires changes in at least three places: the function itself, the Zod
schema, and the API endpoint handler.
...
the CLI package the single source of truth that both surfaces consume.
