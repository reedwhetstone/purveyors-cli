...
- `catalog`: `search` (filters: origin, process, price-min/max, flavor, name, supplier, ids, stocked, variety, drying-method, stocked-days, processing-base-method, fermentation-type, process-additive, processing-confidence-min, sort, offset, limit; proof output via `--include-proof`), `get <id>`, `stats`, `similar <id>`. Structured processing filters require the `member` role under the current session-authenticated CLI path.
